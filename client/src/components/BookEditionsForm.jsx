...
export default function BookEditionsForm({ product, onSaved }) {
  ...
  return <section className="book-editions-panel"><div className="book-editions-heading"><div><span className="eyebrow">PAPERBACK, HARDCOVER &amp; DIGITAL</span><h2>Manage Editions</h2><p>Add the formats customers can purchase for this book. Each format has its own price, stock, ISBN, and delivery details.</p></div><button type="button" onClick={addEdition}><Plus size={16} /> Add Format</button></div><form onSubmit={save}>{editions.map((edition, index) => <div className="book-edition-row" key={`${edition.format}-${index}`}><select name="format" value={edition.format} onChange={(event) => change(index, event)}><option value="paperback">Paperback</option><option value="hardcover">Hardcover</option><option value="pdf">PDF</option><option value="epub">EPUB</option></select><input name="label" placeholder="Format label" value={edition.label || ""} onChange={(event) => change(index, event)} /><input type="number" min="0" name="price" placeholder="Price" value={edition.price} onChange={(event) => change(index, event)} /><input type="number" min="0" name="salePrice" placeholder="Sale price" value={edition.salePrice ?? ""} onChange={(event) => change(index, event)} /><input type="number" min="0" name="stock" placeholder="Stock" value={edition.stock} onChange={(event) => change(index, event)} /><input name="isbn" placeholder="ISBN" value={edition.isbn || ""} onChange={(event) => change(index, event)} /><input name="sku" placeholder="SKU" value={edition.sku || ""} onChange={(event) => change(index, event)} /><input type="number" min="0" step="0.01" name="shippingWeight" placeholder="Weight kg" value={edition.shippingWeight} onChange={(event) => change(index, event)} /><button type="button" aria-label="Remove format" onClick={() => removeEdition(index)}><Trash2 size={16} /></button></div>)}<button className="primary" type="submit" disabled={saving}><Save size={16} /> {saving ? "Saving..." : "Save Editions"}</button>{message && <p className="inline-toast success" role="status">{message}</p>}</form></section>;
}
