...
export default function BookUploadForm({ onSubmit, editingProduct = null }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      const editions = formats.map((item) => ({ ...item, price: Number(item.price || 0), salePrice: item.salePrice === "" ? null : Number(item.salePrice), stock: ["pdf", "epub"].includes(item.format) ? 0 : Number(item.stock || 0), shippingWeight: Number(item.shippingWeight || 0) }));
      // The storefront uses product.price for previews and filtering. Keep it
      // aligned with the least expensive format that has a selling price.
      // Empty optional editions are represented as zero and must not make a
      // paid book appear free.
      const pricedEditions = editions.filter((item) => item.price > 0);
      data.append("price", String(pricedEditions.length ? Math.min(...pricedEditions.map((item) => item.price)) : 0));
      data.append("stock", String(editions.filter((item) => ["paperback", "hardcover"].includes(item.format)).reduce((total, item) => total + item.stock, 0)));
      data.append("editions", JSON.stringify(editions));
