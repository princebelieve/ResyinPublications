...
export default function BookUploadForm({ onSubmit, editingProduct = null }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      const editions = formats.map((item) => ({ ...item, price: Number(item.price || 0), salePrice: item.salePrice === "" ? null : Number(item.salePrice), stock: Number(item.stock || 0), shippingWeight: Number(item.shippingWeight || 0) }));
      ...
      data.append("stock", String(editions.reduce((total, item) => total + item.stock, 0)));
